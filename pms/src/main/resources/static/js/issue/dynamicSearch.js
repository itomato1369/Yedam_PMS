...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll(".add-condition").forEach((menuItem) => {
    menuItem.addEventListener("click", function (event) {
      ...
      // this. 현재 event가 발생한 클래스 .add-conditon의 요소 data-target="이 부분을 가져옴"
      const targetId = this.getAttribute("data-target");
      const targetDiv = document.getElementById(targetId);

      if (targetDiv) {
        // fragment의 id가 targetDiv인 div 조각의 class d-none을 지운다
  ...
  */
  ...
  // id가 searchForm인 태그 가져옴
  const reportForm = document.getElementById("searchForm");
  if (reportForm) {
...
/* 내 일감만 보기 함수만 DOMContentLoaded 밖으로 나온 이유
